package com.instil.dsl.iteration6

@DslMarker
annotation class CourseOutlineMarker
...
abstract class ContainerNode<T : Node> : Node {
    ...
    protected fun renderChildren(builder: StringBuilder): StringBuilder {
...
class Module(val title: String) : ContainerNode<Topic>() {
    override fun render(builder: StringBuilder): StringBuilder {
        builder.append("\tModule called '$title'\n")
        return renderChildren(builder)
    }

    operator fun String.unaryPlus() = children.add(Topic(this))
}

class Course(val title: String) : ContainerNode<Module>() {
